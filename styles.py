...
def apply_style():
    ...
    RC2.69 시절의 남색 사이드바/흰색 메뉴 톤을 복구하고,
    사이드바 메뉴 정렬 CSS가 본문 버튼/도면 버튼에 침범하지 않도록
    적용 범위를 section[data-testid='stSidebar'] 내부로만 제한한다.
    """
    st.markdown(f"""
    <style>
    /* -------------------------
    ...
    /* -------------------------
       Sidebar: RC2.69 navy theme
       ------------------------- */
    section[data-testid="stSidebar"] {{
        background: linear-gradient(180deg, {SIDEBAR_BG} 0%, {SIDEBAR_BG_2} 100%) !important;
    ...
    section[data-testid="stSidebar"] div[data-testid="stButton"] button[kind="secondary"] {{
        ...
        text-align: left !important;
        padding: 0.36rem 0.45rem 0.36rem 0.45rem !important;
    ...
    .map-card {{background:#fff;border:1px solid {BORDER};border-radius:18px;padding:16px;box-shadow:0 8px 24px rgba(15,23,42,.06);overflow-x:auto;overflow-y:visible;max-width:100%;}}
    .map-card > div {{max-width: none !important;transform:scale(0.90);transform-origin:top left;}}
    .rack-title {{text-align:center;font-size:11px;color:#64748b;font-weight:800;margin-top:-13px;height:0;overflow:visible;}}
    ...
    .gbox {{border:1px solid {BORDER};border-radius:12px;overflow:hidden;background:#fff;}}
    .blank-g{{height:78px;border-bottom:1px solid {BORDER}}}.box-label{{text-align:center;padding:10px;font-weight:900;}}
    .center-label{{text-align:center;font-weight:900;font-size:13px;margin-top:8px;color:#111827}}
    ...
    .detail-card {{background:white;border:1px solid {BORDER};border-radius:14px;padding:12px;margin:8px 0;box-shadow:0 5px 16px rgba(15,23,42,.05);}}
    .card-top {{display:flex;justify-content:space-between;align-items:center;gap:8px;}}
    .company-badge {{display:inline-block;background:#eff6ff;color:#1d4ed8!important;font-weight:800;border-radius:999px;padding:3px 8px;font-size:12px;}}
    ...
    .map-card [data-testid="stHorizontalBlock"] {{
