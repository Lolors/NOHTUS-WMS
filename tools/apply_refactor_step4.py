"""Apply refactor step 4 to app.py safely.

This script moves selected product/master-data functions from app.py to
nohtus/services/products.py, then imports them back into app.py.

Target functions are intentionally conservative. If a function is not found,
the script stops so you can inspect the current app.py before continuing.

Run from the repository root after step 1, 2, and 3:
...
APP = ROOT / "app.py"
SERVICE = ROOT / "nohtus" / "services" / "products.py"
BACKUP_APP = ROOT / "app.py.bak_refactor_step4"
BACKUP_SERVICE = ROOT / "nohtus" / "services" / "products.py.bak_refactor_step4"

TARGET_FUNCTIONS = [
    "product_master_excel_bytes",
    "import_product_master_excel",
    "product_options",
]

IMPORT_LINE = "from nohtus.services.products import product_master_excel_bytes, import_product_master_excel, product_options\n"
SERVICE_HEADER = '''"""Product and product-matching service functions for NOHTUS WMS.
...
def main() -> None:
    ...
    if IMPORT_LINE not in app_text:
        ...
        for anchor in anchor_candidates:
            if anchor in app_text:
                ...
                print("ADD product service import")
                break
        else:
            raise SystemExit("Import anchor not found. Stop without modifying app.py.")
    else:
        print("SKIP product service import already present")
    ...
    try:
        ...
        print("OK compile: nohtus/services/products.py")
