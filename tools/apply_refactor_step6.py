"""Apply refactor step 6 to app.py safely.

This script moves page_history() from app.py to nohtus/pages/history.py,
...
APP = ROOT / "app.py"
PAGE = ROOT / "nohtus" / "pages" / "history.py"
BACKUP_APP = ROOT / "app.py.bak_refactor_step6"
BACKUP_PAGE = ROOT / "nohtus" / "pages" / "history.py.bak_refactor_step6"

TARGET_FUNCTION = "page_history"
IMPORT_LINE = "from nohtus.pages.history import page_history\n"
PAGE_HEADER = '''"""History page for NOHTUS WMS.
...
def main() -> None:
    ...
    if IMPORT_LINE not in app_text:
        ...
        for anchor in anchor_candidates:
            if anchor in app_text:
                ...
                inserted = True
                print("ADD history page import")
                break
        if not inserted:
            raise SystemExit("Import anchor not found. Stop without modifying app.py.")
    else:
        print("SKIP history page import already present")
    ...
    try:
        ...
        print("OK compile: nohtus/pages/history.py")
