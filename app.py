...
def move_inventory(src_id, to_company, to_location, qty, memo=""):
    ...
    with connect() as con:
        ...
        row = cur.execute("""SELECT id, qty FROM inventory WHERE company=? AND product_name=? AND IFNULL(warehouse_name,'')=? AND lot=? AND exp_date=? AND location=?""",
                          (to_company, src["product_name"], src.get("warehouse_name") or "", src["lot"], src["exp_date"], to_location)).fetchone()
        if row:
            cur.execute("UPDATE inventory SET qty=?, updated_at=? WHERE id=?", (row[1]+qty, now, row[0]))
        else:
            cur.execute("""INSERT INTO inventory(company,product_name,warehouse_name,lot,exp_date,location,qty,updated_at)
                           VALUES(?,?,?,?,?,?,?,?)""", (to_company, src["product_name"], src.get("warehouse_name"), src["lot"], src["exp_date"], to_location, qty, now))
        tx_type = "사업장+위치이동"
        ...
        insert_transaction_log(cur, created_at=now, tx_type=tx_type, product_name=src["product_name"], warehouse_name=src.get("warehouse_name"),
