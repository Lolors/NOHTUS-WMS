...
class LocationMapEditorIntegrationTests(unittest.TestCase):
    ...
    def test_admin_menu_is_under_basic_section_and_shippable_menu_is_removed(self):
        ...
        registry_source = (Path(__file__).resolve().parents[1] / 'nohtus' / 'pages' / 'registry.py').read_text(encoding='utf-8')
        self.assertIn('"로케이션맵 편집": _page_location_map_editor_guarded,', registry_source)
        self.assertIn('page_location_map_editor()', registry_source)
        self.assertNotIn('page_shippable_inventory', registry_source)
        self.assertNotIn('"출고가능 관리"', registry_source)
    ...
    def test_editor_supports_shape_transform_and_new_location_setup(self):
        ...
        self.assertIn('function fillBackground(type,color,company)', editor_html)
        ...
        self.assertIn("key==='z'", editor_html)
        self.assertIn("key==='y'", editor_html)
        ...
        self.assertIn("else if(!['line','polyline'].includes(shapeType))el.style.background=fillBackground", editor_html)
