...
class InboundLocationAndPackingPresetTests(unittest.TestCase):
    ...
    def test_current_ctn_layout_and_management_labels(self) -> None:
        ...
        self.assertIn("st.markdown('##### 박스 번호 변경')", source)
        self.assertNotIn("with st.expander('박스 번호 변경')", source)
        self.assertNotIn("with st.expander('선택 제품 박스에서 빼기')", source)
        self.assertIn("f'↩ 박스에서 빼기 ({len(remove_item_ids)})'", source)
        self.assertIn("st.columns([1, 1.4, 1])", source)
        ...
        self.assertIn("st.markdown('##### CTN 구성 복제')", source)
        self.assertNotIn("with st.expander('CTN 구성 복제')", source)
