...
import nohtus.pages.export_waiting_compat as export_waiting_compat
...
class ApplicationExportWaitingPatchRestoreTests(unittest.TestCase):
    """수출대기 화면 진입 시 export_waiting_compat.page_export_waiting()이 출고지시
    ...
    def test_restores_patches_after_successful_render(self):
        with patch.object(export_waiting_compat, "_page_export_waiting", return_value="rendered") as mock_render:
            result = export_waiting_compat.page_export_waiting()
    ...
    def test_restores_patches_when_render_raises(self):
        with patch.object(export_waiting_compat, "_page_export_waiting", side_effect=RuntimeError("boom")):
            with self.assertRaisesRegex(RuntimeError, "boom"):
                export_waiting_compat.page_export_waiting()
    ...
    def test_take_p_is_patched_only_during_render(self):
        ...
        with patch.object(export_waiting_compat, "_page_export_waiting", side_effect=capture_take_p):
            export_waiting_compat.page_export_waiting()
