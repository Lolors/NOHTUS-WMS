...
def render_inbound_quick_location_map():
    ...
    새 창/새 탭/URL 이동 없이 현재 Streamlit 화면의 숨김 입력칸과 적용 버튼만 사용해
    입고 위치 선택기를 동기화한다.
    """
    ...
    def cell(loc, text=None):
        ...
        return f'<button type="button" class="map-cell{cls}" data-inbound-loc="{escape(loc)}">{escape(text)}</button>'
    ...
    def zone(loc, text, left, top, w, h, cls="white", extra=""):
        ...
        return f'<button type="button" class="zone {cls}{selected_cls}" data-inbound-loc="{escape(loc)}" style="left:{left}px;top:{top}px;width:{w}px;height:{h}px;{extra}">{text}</button>'
...
.map-scroll{{overflow:visible;height:670px;padding:0;}}
.map-stage{{position:relative;width:1160px;height:704px;min-width:1160px;background:#fff;border-radius:14px;transform:scale(0.88);transform-origin:top left;}}
...
.map-cell,.zone{{appearance:none;position:relative;display:flex;align-items:center;justify-content:center;text-decoration:none;color:#0f172a;font-weight:900;font-size:14px;border:0;border-right:1px solid rgba(51,65,85,.38);border-bottom:1px solid rgba(51,65,85,.38);cursor:pointer;background:#fff;}}
...
.special-menu button{{appearance:none;border:1px solid #e2e8f0;background:#f8fafc;border-radius:9px;padding:8px 7px;font-size:12px;font-weight:900;color:#0f172a;cursor:pointer;text-align:center;}}
.special-menu button:hover,.special-menu button.selected{{background:#22c55e;color:#fff;border-color:#16a34a;}}
...
.big-left button{{appearance:none;position:relative;display:flex;align-items:center;justify-content:center;width:100%;border:0;border-bottom:1px solid #cbd5e1;background:#f7f8fa;color:#0f172a;font-weight:900;cursor:pointer;}}
.big-left button:hover{{outline:3px solid rgba(37,99,235,.22);z-index:2;}}
.big-left button.selected{{background:#22c55e!important;color:#fff!important;outline:3px solid rgba(34,197,94,.35)!important;box-shadow:0 0 0 3px rgba(255,255,255,.8),0 0 18px rgba(34,197,94,.75)!important;z-index:4;}}
.g2{{height:225px;background:#f7f8fa;}} .g1row{{height:57px;display:grid;grid-template-columns:1fr 1fr 1fr;}}
.g1row button{{height:57px;border-right:1px solid #cbd5e1;border-bottom:none;}} .g1row button:last-child{{border-right:none;}}
...
.qp button{{position:relative;display:grid;grid-template-columns:58px 1fr;align-items:center;width:100%;height:74px;border:0;border-bottom:1px solid #e2e8f0;background:#fff;color:#111827;font-weight:900;cursor:pointer;text-align:left;}}
...
<div class="inbound-map-card">
  ...
  <div class="map-scroll"><div class="map-stage">
    <div class="big-left">
      <button type="button" class="g2 gray{' selected' if is_selected('G2') else ''}" data-inbound-loc="G2">G2</button>
    ...
    <div class="qp">
      <button type="button" class="{'selected' if is_selected('Q') or is_selected('Q1') or is_selected('Q2') else ''}" data-inbound-loc="Q"><span class="qp-key qkey">Q</span><span>유통기간임박</span></button>
      <button type="button" class="{'selected' if is_selected('P') else ''}" data-inbound-loc="P"><span class="qp-key">P</span><span>수출대기</span></button>
    </div>
    ...
    <div class="special-menu" id="inboundSpecialMenu" style="left:975px;top:492px;width:168px;">
      <button type="button" data-inbound-loc="홍보물랙">홍보물랙</button>
      <button type="button" data-inbound-loc="회색 카트">회색 카트</button>
      <button type="button" data-inbound-loc="오른쪽 창고">오른쪽 창고</button>
      <button type="button" data-inbound-loc="사무실(4층)">사무실(4층)</button>
    </div>
  </div></div>
</div>
<script>
