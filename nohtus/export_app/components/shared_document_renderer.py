...
def _print_title_script(title: str, function_name: str = 'printFinalDocument') -> str:
    """window.print()를 부르기 전에 문서 제목을 바꿔서, 브라우저의 'PDF로 저장'
    인쇄 대화상자가 이 제목을 파일명으로 제안하게 한다(JPG 저장과 같은 규칙)."""
...
<div class="toolbar"><button class="download-jpg" onclick="downloadFinalDocumentJpg()">🖼️ JPG</button><button class="print" onclick="printFinalDocument()">🖨 출력하기</button></div>
...
def render_shipment_product_list(case, actual_rows) -> None:
    ...
    document_title = html.escape(raw_document_title)
    print_title_script = _print_title_script(raw_document_title, 'printShipmentList')
    jpg_download_script = _jpg_download_script(
...
<div class="toolbar"><button class="download-jpg" onclick="downloadShipmentListJpg()">🖼️ JPG</button><button class="print" onclick="printShipmentList()">🖨 출력하기</button></div>
