...
from nohtus.export_app.services import export_service, folder_service, history_service, usb_storage_service
...
folder_col, usb_col, current_col = st.columns(3, gap='large')
...
with usb_col:
    ...
    if st.button('선택 경로의 드라이브를 수출 USB로 등록', use_container_width=True):
        ok, message = check_folder_path(folder_text)
        if not ok:
            st.error(message)
        else:
