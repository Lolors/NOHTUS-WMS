...
MENU_SECTIONS = [
    ...
    ("수출", [
        "주문 입력", "주문 검색 및 수정", "수출대기 저장", "수출확정 매출 등록",
        "박스 패킹", "국내배송", "공유용 자료", "기간별 통계", "내 폴더",
    ]),
...
HIDDEN_PAGES = {"재고 찾기": "모바일용 재고 찾기 메뉴는 임시 숨김"}
...
def render_sidebar(app_title, version, allowed_pages=None):
    ...
    def submenu_button(label):
        active = st.session_state.get("page") == label
        if st.button(label, use_container_width=True, type="primary" if active else "secondary", key=f"export_submenu_btn_{label}"):
            _go_to(label)
    ...
    def render_collapsible_section(section, labels):
        ...
        with st.sidebar.container():
            ...
            if not expanded and toggle_col.button("펼치기", key="export_submenu_expand", use_container_width=True):
