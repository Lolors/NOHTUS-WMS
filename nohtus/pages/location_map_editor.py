...
def page_location_map_editor():
    ...
    *{{box-sizing:border-box}} body{{margin:0;font-family:Inter,Segoe UI,Arial,'Noto Sans KR',sans-serif;background:#f8fafc;color:#0f172a}}
      ...
      <aside class="panel">
        ...
        <section id="changeTab" class="tab-panel">
          ...
          <div id="selectedFields" hidden>
            ...
            <div class="panel-actions"><button type="button" id="bringToFront">맨 위로</button><button type="button" id="sendToBack">맨 뒤로</button></div><div class="panel-actions"><button type="button" class="danger" id="deleteSelected">삭제</button></div><p class="hint">겹쳐서 선택하기 어려운 항목은 Alt를 누른 채 클릭하면 위에서 아래 순서로 선택할 수 있습니다.</p>
            <h3 class="location-only">균등 분할</h3>
            <div class="field-row location-only"><div class="field"><label for="splitCount">분할 후 칸 수</label><input id="splitCount" type="number" value="2" min="2" max="100"></div><div class="field"><label for="splitAxis">분할 방향</label><select id="splitAxis"><option value="x">가로</option><option value="y">세로</option></select></div></div>
            ...
            <button type="button" class="primary location-only" id="splitSelected" style="width:100%">선택 도형 균등 분할</button><p class="hint location-only">입력한 숫자가 최종 칸 수입니다. 예: 4 입력 → 분할선 3개, 결과 4칸</p>
    ...
    function handles(){{return ['nw','n','ne','e','se','s','sw','w'].map(dir=>`<i class="resize-handle resize-${{dir}}" data-dir="${{dir}}"></i>`).join('')+'<i class="rotate-line"></i><i class="rotate-handle"></i>';}}function resizeCursor(dir,rotation){{const vectors={{n:[0,-1],ne:[1,-1],e:[1,0],se:[1,1],s:[0,1],sw:[-1,1],w:[-1,0],nw:[-1,-1]}},v=vectors[dir]||[1,0],degrees=((Math.atan2(v[1],v[0])*180/Math.PI+Number(rotation||0))%180+180)%180,bucket=Math.round(degrees/45)%4;return ['ew-resize','nwse-resize','ns-resize','nesw-resize'][bucket];}}function applyHandleCursors(el,rotation){{el.querySelectorAll('.resize-handle').forEach(handle=>handle.style.cursor=resizeCursor(handle.dataset.dir,rotation));}}
    ...
    function updateElement(el,it){{el.style.left=it.x+'px';el.style.top=it.y+'px';el.style.width=it.width+'px';el.style.height=it.height+'px';el.style.transform=`rotate(${{Number(it.rotation||0)}}deg)`;const label=el.querySelector('span');if(label)label.style.transform=`rotate(${{-Number(it.rotation||0)}}deg)`;applyHandleCursors(el,it.rotation);syncPanel();}}
    ...
    stage.addEventListener('pointermove',e=>{{if(!action)return;if(action.mode==='marquee'){{const p=stagePoint(e);action.x=p.x;action.y=p.y;const left=Math.min(action.sx,p.x),top=Math.min(action.sy,p.y);action.box.style.left=left+'px';action.box.style.top=top+'px';action.box.style.width=Math.abs(p.x-action.sx)+'px';action.box.style.height=Math.abs(p.y-action.sy)+'px';return}}const it=layout.items[action.idx];const dx=(e.clientX-action.sx)/scale,dy=(e.clientY-action.sy)/scale;if(action.mode==='move'){{action.members.forEach(m=>{{const target=layout.items[m.idx];target.x=clamp(snap(m.x+dx),0,layout.canvas.width-target.width);target.y=clamp(snap(m.y+dy),0,layout.canvas.height-target.height);const node=stage.querySelector(`.loc[data-idx="${{m.idx}}"]`);if(node)updateElement(node,target);}});showSmartGuides(it,action.idx);}}
      ...
      else{{const resizeAngle=action.rotation*Math.PI/180,cosResize=Math.cos(resizeAngle),sinResize=Math.sin(resizeAngle),resizeDx=dx*cosResize+dy*sinResize,resizeDy=-dx*sinResize+dy*cosResize,dir=action.dir;let w=action.width,h=action.height;if(dir.includes('e'))w=snap(action.width+resizeDx);if(dir.includes('w'))w=snap(action.width-resizeDx);if(dir.includes('s'))h=snap(action.height+resizeDy);if(dir.includes('n'))h=snap(action.height-resizeDy);w=Math.max(minWidth,w);h=Math.max(minHeight,h);const shiftLocalX=dir.includes('e')?(w-action.width)/2:dir.includes('w')?(action.width-w)/2:0,shiftLocalY=dir.includes('s')?(h-action.height)/2:dir.includes('n')?(action.height-h)/2:0,shiftWorldX=shiftLocalX*cosResize-shiftLocalY*sinResize,shiftWorldY=shiftLocalX*sinResize+shiftLocalY*cosResize,startCenterX=action.x+action.width/2,startCenterY=action.y+action.height/2,x=Math.round(startCenterX+shiftWorldX-w/2),y=Math.round(startCenterY+shiftWorldY-h/2);Object.assign(it,{{x,y,width:w,height:h}});showSmartGuides(it,action.idx);}}
    ...
    document.getElementById('splitSelected').addEventListener('click',()=>{{if(selectedIndex===null)return;const source={{...layout.items[selectedIndex]}};const cellCount=clamp(Math.floor(Number(document.getElementById('splitCount').value)||2),2,100);const axis=document.getElementById('splitAxis').value;const gap=Math.max(0,snap(Number(document.getElementById('splitGap').value)||0));const total=axis==='x'?source.width:source.height;const cell=snap((total-gap*(cellCount-1))/cellCount);if((axis==='x'&&cell<minWidth)||(axis==='y'&&cell<minHeight)){{status.textContent='도형 크기에 비해 최종 칸 수 또는 간격이 너무 큽니다.';return}}const prefix=document.getElementById('splitPrefix').value.trim()||String(source.code||'LOC').replace(/[-_ ]?\d+$/,'');checkpoint();const group='group-'+Date.now();const made=[],reserved=new Set(layout.items.filter((_,i)=>i!==selectedIndex).map(it=>String(it.code||'').trim().toLocaleUpperCase()));function claimCode(n){{let i=n,code='';do{{code=`${{prefix}}-${{String(i).padStart(2,'0')}}`;i++;}}while(reserved.has(code.toLocaleUpperCase()));reserved.add(code.toLocaleUpperCase());return code;}}for(let i=0;i<cellCount;i++){{const it={{...source,group_id:group,code:claimCode(i+1)}};it.label=it.code;if(axis==='x'){{it.x=source.x+i*(cell+gap);it.width=i===cellCount-1?source.x+source.width-it.x:cell;}}else{{it.y=source.y+i*(cell+gap);it.height=i===cellCount-1?source.y+source.height-it.y:cell;}}made.push(it);}}layout.items.splice(selectedIndex,1,...made);selectedIndices.clear();made.forEach((_,i)=>selectedIndices.add(selectedIndex+i));draw();status.textContent=`${{source.code}}을(를) ${{cellCount}}개로 균등 분할했습니다.`;}});
    ...
    function moveSelectionLayer(toFront){{if(!selectedIndices.size)return;checkpoint();const primary=selectedIndex===null?null:layout.items[selectedIndex],chosen=layout.items.filter((_,i)=>selectedIndices.has(i)),remaining=layout.items.filter((_,i)=>!selectedIndices.has(i)),ordered=toFront?[...remaining,...chosen]:[...chosen,...remaining],chosenSet=new Set(chosen);layout.items.splice(0,layout.items.length,...ordered);selectedIndices.clear();layout.items.forEach((it,i)=>{{if(chosenSet.has(it))selectedIndices.add(i);}});selectedIndex=primary?layout.items.indexOf(primary):(selectedIndices.values().next().value??null);draw();status.textContent=`선택한 ${{chosen.length}}개 항목을 ${{toFront?'맨 위':'맨 뒤'}}로 이동했습니다.`;}}document.getElementById('bringToFront').addEventListener('click',()=>moveSelectionLayer(true));document.getElementById('sendToBack').addEventListener('click',()=>moveSelectionLayer(false));
