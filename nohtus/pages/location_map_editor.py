...
def page_location_map_editor():
    ...
    *{{box-sizing:border-box}} body{{margin:0;font-family:Inter,Segoe UI,Arial,'Noto Sans KR',sans-serif;background:#f8fafc;color:#0f172a}}
      ...
      <aside class="panel">
        ...
        <section id="changeTab" class="tab-panel">
          ...
          <div id="selectedFields" hidden>
            ...
            <p class="hint">일반 로케이션은 재고가 연결되는 실제 보관 위치이고, 구역은 통로·작업장 같은 지도 표시용 영역입니다.</p>
        ...
        <section id="newTab" class="tab-panel" hidden>
          ...
          <p class="hint">일반 로케이션은 재고 연결용, 구역은 지도 안내용입니다.</p>
    ...
    document.getElementById('deleteSelected').addEventListener('click',()=>{{if(selectedIndex===null)return;const code=layout.items[selectedIndex].code;if(!confirm(`${{code}} 항목을 삭제할까요?`))return;const targets=Array.from(selectedIndices).sort((a,b)=>b-a);targets.forEach(i=>layout.items.splice(i,1));selectedIndices.clear();selectedIndex=null;draw();status.textContent=`${{code}} 삭제됨`;}});
