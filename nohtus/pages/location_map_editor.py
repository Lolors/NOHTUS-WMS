...
def page_location_map_editor():
    ...
    *{{box-sizing:border-box}} body{{margin:0;font-family:Inter,Segoe UI,Arial,'Noto Sans KR',sans-serif;background:#f8fafc;color:#0f172a}}
    ...
    const grid=Number(layout.canvas.grid||10); const minWidth=36,minHeight=30; let scale=.82,action=null,selectedIndex=null; const guideTolerance=Math.max(4,grid/2);
    ...
    function showSmartGuides(it,idx){{clearGuides();const others=layout.items.filter((_,i)=>i!==idx);const xs=[it.x,it.x+it.width/2,it.x+it.width],ys=[it.y,it.y+it.height/2,it.y+it.height];const gx=new Set(),gy=new Set();others.forEach(o=>{{[o.x,o.x+o.width/2,o.x+o.width].forEach(v=>xs.forEach(a=>{{if(Math.abs(v-a)<=guideTolerance)gx.add(v)}}));[o.y,o.y+o.height/2,o.y+o.height].forEach(v=>ys.forEach(a=>{{if(Math.abs(v-a)<=guideTolerance)gy.add(v)}}));}});gx.forEach(v=>addGuide('x',v));gy.forEach(v=>addGuide('y',v));
    ...
    stage.addEventListener('pointermove',e=>{{if(!action)return;const it=layout.items[action.idx];const dx=(e.clientX-action.sx)/scale,dy=(e.clientY-action.sy)/scale;if(action.mode==='move'){{it.x=clamp(snap(action.x+dx),0,layout.canvas.width-it.width);it.y=clamp(snap(action.y+dy),0,layout.canvas.height-it.height);showSmartGuides(it,action.idx);}}
    ...
    document.getElementById('splitSelected').addEventListener('click',()=>{{if(selectedIndex===null)return;const source={{...layout.items[selectedIndex]}};const count=clamp(Math.floor(Number(document.getElementById('splitCount').value)||2),2,100);const axis=document.getElementById('splitAxis').value;const gap=Math.max(0,snap(Number(document.getElementById('splitGap').value)||0));const total=axis==='x'?source.width:source.height;const cell=snap((total-gap*(count-1))/count);if((axis==='x'&&cell<minWidth)||(axis==='y'&&cell<minHeight)){{status.textContent='도형 크기에 비해 분할 개수 또는 간격이 너무 큽니다.';return}}const prefix=document.getElementById('splitPrefix').value.trim()||String(source.code||'LOC').replace(/[-_ ]?\d+$/,'');const group='group-'+Date.now();const made=[];for(let i=0;i<count;i++){{const it={{...source,group_id:group,code:nextCode(prefix,i+1)}};it.label=it.code;if(axis==='x'){{it.x=source.x+i*(cell+gap);it.width=i===count-1?source.x+source.width-it.x:cell;}}else{{it.y=source.y+i*(cell+gap);it.height=i===count-1?source.y+source.height-it.y:cell;}}made.push(it);}}layout.items.splice(selectedIndex,1,...made);draw();status.textContent=`${{source.code}}을(를) ${{count}}개로 균등 분할했습니다.`;}});
    document.getElementById('duplicateSelected').addEventListener('click',()=>{{if(selectedIndex===null)return;const source=layout.items[selectedIndex];const members=source.group_id?layout.items.filter(it=>it.group_id===source.group_id):[source];const newGroup='group-'+Date.now();const copies=members.map((it,i)=>{{const copy={{...it,x:clamp(it.x+grid*2,0,layout.canvas.width-it.width),y:clamp(it.y+grid*2,0,layout.canvas.height-it.height),code:nextCode(String(it.code||'LOC')+'-COPY',1)}};if(source.group_id)copy.group_id=newGroup;return copy;}});layout.items.push(...copies);selectedIndex=layout.items.length-copies.length;draw();status.textContent=`${{copies.length}}개 도형을 묶음 복제했습니다.`;}});
