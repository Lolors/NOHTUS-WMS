...
def page_location_map_editor():
    ...
    *{{box-sizing:border-box}} body{{margin:0;font-family:Inter,Segoe UI,Arial,'Noto Sans KR',sans-serif;background:#f8fafc;color:#0f172a}}
    ...
    <div class="editor-shell">
      ...
      <aside class="panel">
        <div class="tabs"><button type="button" class="tab-button active" data-tab="changeTab">선택 항목 변경</button><button type="button" class="tab-button" data-tab="newTab">새 항목 추가</button></div>
        <section id="changeTab" class="tab-panel">
          ...
          <div id="selectedFields" hidden>
            ...
            <p class="hint">일반 로케이션은 개별 보관 위치, 구역은 여러 위치를 대표하는 큰 영역용 분류입니다. 현재 저장·지도 클릭 동작은 동일합니다.</p>
            <div class="panel-actions"><button type="button" class="primary" id="applyProperties">속성 적용</button><button type="button" id="duplicateSelected">묶음 복제</button></div>
            <div class="panel-actions"><button type="button" class="danger" id="deleteSelected">삭제</button></div>
            <h3>균등 분할</h3>
            <div class="field-row"><div class="field"><label for="splitCount">분할 개수</label><input id="splitCount" type="number" value="2" min="2" max="100"></div><div class="field"><label for="splitAxis">분할 방향</label><select id="splitAxis"><option value="x">가로</option><option value="y">세로</option></select></div></div>
            <div class="field-row"><div class="field"><label for="splitGap">칸 간격</label><input id="splitGap" type="number" value="0" min="0"></div><div class="field"><label for="splitPrefix">코드 접두어</label><input id="splitPrefix" placeholder="예: A3"></div></div>
            <button type="button" class="primary" id="splitSelected" style="width:100%">선택 도형 균등 분할</button>
    ...
    const grid=Number(layout.canvas.grid||10); const minWidth=36,minHeight=30; let scale=.82,action=null,selectedIndex=null; const selectedIndices=new Set(); const guideTolerance=Math.max(4,grid/2);
    ...
    function draw(){{stage.innerHTML='';stage.style.width=layout.canvas.width+'px';stage.style.height=layout.canvas.height+'px';stage.style.transform=`scale(${{scale}})`;stage.style.transformOrigin='top left';layout.items.forEach((it,idx)=>{{
      const el=document.createElement('div');el.className=`loc ${{palette(it.company)}}${{selectedIndices.has(idx)?' selected':''}}`;el.dataset.idx=idx;el.style.left=it.x+'px';el.style.top=it.y+'px';el.style.width=it.width+'px';el.style.height=it.height+'px';el.style.transform=`rotate(${{Number(it.rotation||0)}}deg)`;
      const label=document.createElement('span');label.textContent=it.label||it.code;el.append(label);el.insertAdjacentHTML('beforeend',handles());stage.appendChild(el);
    }});syncPanel();}}
    function syncPanel(){{const selected=selectedIndex!==null&&layout.items[selectedIndex];document.getElementById('emptySelection').hidden=!!selected;document.getElementById('selectedFields').hidden=!selected;if(!selected)return;fields.propCode.value=selected.code||'';fields.propLabel.value=selected.label||selected.code||'';fields.propCompany.value=selected.company||'기타';fields.propKind.value=selected.kind||'location';}}
    ...
    function updateElement(el,it){{el.style.left=it.x+'px';el.style.top=it.y+'px';el.style.width=it.width+'px';el.style.height=it.height+'px';el.style.transform=`rotate(${{Number(it.rotation||0)}}deg)`;syncPanel();}}
    stage.addEventListener('pointerdown',e=>{{const el=e.target.closest('.loc');if(!el)return;e.preventDefault();const idx=Number(el.dataset.idx),multi=e.ctrlKey||e.shiftKey;if(multi){{if(selectedIndices.has(idx)&&selectedIndices.size>1)selectedIndices.delete(idx);else selectedIndices.add(idx);selectedIndex=selectedIndices.has(idx)?idx:(selectedIndices.values().next().value??null);draw();return}}if(!selectedIndices.has(idx)){{selectedIndices.clear();selectedIndices.add(idx);selectedIndex=idx;draw();}}else selectedIndex=idx;const activeEl=stage.querySelector(`.loc[data-idx="${{idx}}"]`);const it=layout.items[idx];const handle=e.target.closest('.resize-handle');const rotate=e.target.closest('.rotate-handle');const rect=(activeEl||el).getBoundingClientRect();const members=Array.from(selectedIndices).map(i=>({{idx:i,x:layout.items[i].x,y:layout.items[i].y}}));action={{mode:rotate?'rotate':handle?'resize':'move',dir:handle?.dataset.dir||'',el:activeEl||el,idx,members,sx:e.clientX,sy:e.clientY,x:it.x,y:it.y,width:it.width,height:it.height,rotation:Number(it.rotation||0),cx:rect.left+rect.width/2,cy:rect.top+rect.height/2}};(activeEl||el).setPointerCapture(e.pointerId);status.textContent=`${{selectedIndices.size}}개 도형 선택됨`;}});
    stage.addEventListener('pointermove',e=>{{if(!action)return;const it=layout.items[action.idx];const dx=(e.clientX-action.sx)/scale,dy=(e.clientY-action.sy)/scale;if(action.mode==='move'){{action.members.forEach(m=>{{const target=layout.items[m.idx];target.x=clamp(snap(m.x+dx),0,layout.canvas.width-target.width);target.y=clamp(snap(m.y+dy),0,layout.canvas.height-target.height);const node=stage.querySelector(`.loc[data-idx="${{m.idx}}"]`);if(node)updateElement(node,target);}});showSmartGuides(it,action.idx);}}
    ...
    function endAction(){{action=null;clearGuides()}} stage.addEventListener('pointerup',endAction);stage.addEventListener('pointercancel',endAction);
