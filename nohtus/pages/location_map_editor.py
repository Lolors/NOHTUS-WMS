...
def page_location_map_editor():
    ...
    *{{box-sizing:border-box}} body{{margin:0;font-family:Inter,Segoe UI,Arial,'Noto Sans KR',sans-serif;background:#f8fafc;color:#0f172a}}
    ...
    stage.addEventListener('pointermove',e=>{{if(!action)return;if(action.mode==='marquee'){{const p=stagePoint(e);action.x=p.x;action.y=p.y;const left=Math.min(action.sx,p.x),top=Math.min(action.sy,p.y);action.box.style.left=left+'px';action.box.style.top=top+'px';action.box.style.width=Math.abs(p.x-action.sx)+'px';action.box.style.height=Math.abs(p.y-action.sy)+'px';return}}const it=layout.items[action.idx];const dx=(e.clientX-action.sx)/scale,dy=(e.clientY-action.sy)/scale;if(action.mode==='move'){{action.members.forEach(m=>{{const target=layout.items[m.idx];target.x=clamp(snap(m.x+dx),0,layout.canvas.width-target.width);target.y=clamp(snap(m.y+dy),0,layout.canvas.height-target.height);const node=stage.querySelector(`.loc[data-idx="${{m.idx}}"]`);if(node)updateElement(node,target);}});showSmartGuides(it,action.idx);}}
      else if(action.mode==='rotate'){{const start=Math.atan2(action.sy-action.cy,action.sx-action.cx);const now=Math.atan2(e.clientY-action.cy,e.clientX-action.cx);const targetRotation=Math.round((action.rotation+(now-start)*180/Math.PI)/5)*5;action.members.forEach(m=>{{const target=layout.items[m.idx];target.rotation=targetRotation;const node=stage.querySelector(`.loc[data-idx="${{m.idx}}"]`);if(node)updateElement(node,target);}});}}
...
def page_location_map_editor():
    ...
    *{{box-sizing:border-box}} body{{margin:0;font-family:Inter,Segoe UI,Arial,'Noto Sans KR',sans-serif;background:#f8fafc;color:#0f172a}}
    ...
    function isDuplicateCode(code,exceptIndex=null){{const key=code.trim().toLocaleUpperCase();return layout.items.some((it,idx)=>idx!==exceptIndex&&String(it.code||'').trim().toLocaleUpperCase()===key);}}
    function updateElement(el,it){{el.style.left=it.x+'px';el.style.top=it.y+'px';el.style.width=it.width+'px';el.style.height=it.height+'px';el.style.transform=`rotate(${{Number(it.rotation||0)}}deg)`;syncPanel();}}
    stage.addEventListener('pointerdown',e=>{{const el=e.target.closest('.loc');if(!el)return;e.preventDefault();const idx=Number(el.dataset.idx),multi=e.ctrlKey||e.shiftKey;if(multi){{if(selectedIndices.has(idx)&&selectedIndices.size>1)selectedIndices.delete(idx);else selectedIndices.add(idx);selectedIndex=selectedIndices.has(idx)?idx:(selectedIndices.values().next().value??null);draw();return}}if(!selectedIndices.has(idx)){{selectedIndices.clear();selectedIndices.add(idx);selectedIndex=idx;draw();}}else selectedIndex=idx;const activeEl=stage.querySelector(`.loc[data-idx="${{idx}}"]`);const it=layout.items[idx];const handle=e.target.closest('.resize-handle');const rotate=e.target.closest('.rotate-handle');const rect=(activeEl||el).getBoundingClientRect();const members=Array.from(selectedIndices).map(i=>({{idx:i,x:layout.items[i].x,y:layout.items[i].y}}));checkpoint();action={{mode:rotate?'rotate':handle?'resize':'move',dir:handle?.dataset.dir||'',el:activeEl||el,idx,members,sx:e.clientX,sy:e.clientY,x:it.x,y:it.y,width:it.width,height:it.height,rotation:Number(it.rotation||0),cx:rect.left+rect.width/2,cy:rect.top+rect.height/2}};(activeEl||el).setPointerCapture(e.pointerId);status.textContent=`${{selectedIndices.size}}개 도형 선택됨`;}});
    ...
    document.getElementById('undo').addEventListener('click',undo);document.getElementById('redo').addEventListener('click',redo);document.addEventListener('keydown',e=>{{const tag=String(e.target?.tagName||'').toLowerCase(),editing=['input','textarea','select'].includes(tag)||e.target?.isContentEditable;if(!editing&&(e.key==='Delete'||e.key==='Backspace')){{e.preventDefault();deleteSelection(false);return}}if((e.ctrlKey||e.metaKey)&&!editing&&e.key.toLowerCase()==='z'){{e.preventDefault();e.shiftKey?redo():undo();return}}if((e.ctrlKey||e.metaKey)&&!editing&&e.key.toLowerCase()==='y'){{e.preventDefault();redo();}}}});updateHistoryButtons();
