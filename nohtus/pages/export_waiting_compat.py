...
수출대기 화면은 출고지시 렌더러(nohtus.pages.outbound)를 그대로
재사용하는데, 그 사이 함수 시그니처가 여러 번 바뀌면서 생긴 구멍(누락된
호환 함수, 다른 인자 개수 등)을 여기서 메꾼다. 페이지 함수 하나뿐인
호출부(nohtus.pages.registry)만 있으므로 안심하고 확장해도 된다.
"""
