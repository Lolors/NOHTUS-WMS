...
def _customer_options(company, term):
    ...
    if not columns or "customer_name" not in columns:
    ...
    sql = f"SELECT {select_code} AS customer_code, customer_name, {company_col or "''"} AS company FROM customers WHERE {' AND '.join(clauses)} ORDER BY customer_name LIMIT 100"
    return q(sql, tuple(params))


def _order_items(order_id):
    df = q(
        """SELECT company AS 사업장, source_location AS 원래로케이션, waiting_location AS 현재로케이션,
                  product_name AS 제품명, lot AS LOT, exp_date AS 유통기한, qty AS 수량
           FROM export_waiting_items WHERE order_id=? ORDER BY company,product_name,lot,exp_date,source_location""",
        (int(order_id),),
    )
    if not df.empty:
...
def page_saved_export_waiting():
    ...
    orders = q(
        """SELECT id,export_no,country,title,status,erp_company,erp_customer_name,
    ...
    st.dataframe(view[["번호", "상태", "국가", "수출번호", "제목", "ERP사업장", "ERP매출처", "등록일"]],
                 hide_index=True, use_container_width=True)

    labels = [f"#{int(r.id)} | {STATUS_LABELS.get(r.status, r.status)} | {r.title}" for r in orders.itertuples()]
    selected_label = st.selectbox("수출대기 건 선택", labels)
    selected = orders.iloc[labels.index(selected_label)]
    ...
    c3.metric("수출번호", str(selected["export_no"] or "-"))
    ...
    if status == "cancelled":
        st.error("취소된 수출대기 건입니다. 품목은 등록 당시의 원래 로케이션으로 복구되었습니다.")
        return
    if status == "confirmed":
        st.success(f"수출확정 완료 | {selected['erp_company'] or '-'} / {selected['erp_customer_name'] or '-'}")
        return
    ...
    st.markdown("---")
    st.markdown("### 수출확정")
    st.caption("실제 ERP 매출을 등록할 사업장과 수출 매출처를 선택하면 P 재고가 최종 차감됩니다.")
    ...
    customer_term = st.text_input("ERP 수출 매출처 검색", placeholder="매출처명 일부를 입력하세요", key=f"export_customer_term_{order_id}")
    customers = _customer_options(erp_company, customer_term)
    if customers.empty:
        st.warning("선택한 사업장의 ERP 매출처가 없습니다. 거래처 관리에서 ERP 매출처 목록을 먼저 등록하거나 갱신하세요.")
        return
    customer_labels = [f"{str(r.customer_code or '').strip() or '-'} | {r.customer_name}" for r in customers.itertuples()]
    customer_label = st.selectbox("ERP 수출 매출처", customer_labels, key=f"export_customer_select_{order_id}")
    customer = customers.iloc[customer_labels.index(customer_label)]
