...
def _load_inventory(product_term: str, company_filter: str, status_filter: str):
    ...
    sql = f"""SELECT id,COALESCE(is_shippable,1) AS is_shippable,company,product_name,warehouse_name,lot,exp_date,location,qty
              FROM inventory WHERE {' AND '.join(where)}
              ORDER BY company,product_name,location,lot,exp_date,id LIMIT 500"""
...
def page_shippable_inventory():
    ...
    with c1: product_term = st.text_input("검색", placeholder="제품명/ERP명/LOT/로케이션", key="ship_inv_term")
    with c2: company_filter = st.selectbox("사업장", _company_options(), key="ship_inv_company")
    with c3: status_filter = st.selectbox("상태", ["전체","출고가능","출고제외"], key="ship_inv_status")
    ...
    work = work.rename(columns={"id":"ID","is_shippable":"출고가능","company":"사업장","product_name":"표준제품명","warehouse_name":"ERP명","lot":"LOT","exp_date":"유통기한","location":"로케이션","qty":"수량"})
    edited = st.data_editor(work[["ID","출고가능","사업장","로케이션","표준제품명","ERP명","LOT","유통기한","수량"]],hide_index=True,use_container_width=True,num_rows="fixed",disabled=["ID","사업장","로케이션","표준제품명","ERP명","LOT","유통기한","수량"],column_config={"ID":st.column_config.NumberColumn("ID",disabled=True,width="small"),"출고가능":st.column_config.CheckboxColumn("출고가능")},key="ship_inv_editor")
    if not st.button("출고가능 설정 저장", type="primary", use_container_width=True): return
    updates=[]; edited=edited.reset_index(drop=True)
    for pos,row in edited.iterrows():
        inv_id=int(row.get("ID")); new_value=1 if bool(row.get("출고가능",True)) else 0; old_value=1 if bool(work.iloc[pos].get("출고가능",True)) else 0
        if new_value != old_value: updates.append((new_value,inv_id))
    if not updates:
        st.info("변경된 출고가능 설정이 없습니다.")
        return
    with connect() as con:
        con.executemany("UPDATE inventory SET is_shippable=? WHERE id=?", updates); con.commit()
