...
def _export_title():
    buyer = str(st.session_state.get("export_waiting_country") or "").strip()
    export_no = str(st.session_state.get("export_waiting_number") or "").strip()
    if buyer and export_no:
        return f"{buyer}_{export_no}"
    return buyer or export_no or "수출대기"
...
def _load_editing_order():
    ...
    order = q("SELECT country,export_no,shipping_method,title,status FROM export_waiting_orders WHERE id=?", (int(order_id),))
...
def page_export_waiting():
    ...
    def patched_save_outbound_order(cart, title="", memo=""):
        result = save_export_waiting_order(
            ...
            shipping_method=st.session_state.get("export_waiting_shipping_method"),
    ...
    def patched_rerun(*args, **kwargs):
        if export_completed["done"]:
            ...
            for key in ["export_waiting_number", "export_waiting_country", "export_waiting_shipping_method", "export_waiting_auto_title", "export_editing_order_id", "_export_edit_loaded"]:
