...
def parse_kakao_order(text: str) -> ParsedKakaoOrder:
    ...
    # 한 줄 주문: "안양윤정형 콘쥬란 2통 보내주세요" 같은 형태 보조 처리
    if not items and len(lines) == 1:
        compact = _clean_line(_TRAILING_REQUEST_RE.sub("", lines[0]))
        match = re.match(r"^(?P<customer>\S+)\s+(?P<product>.+?)\s*(?P<qty>\d+)\s*(?P<unit>통|개|박스|box|BOX|ea|EA|v|V|바이알|세트|set|SET)?$", compact)
        if match:
            customer = _clean_line(match.group("customer"))
            items.append(
                ParsedOrderItem(
                    product_keyword=_clean_line(match.group("product")),
