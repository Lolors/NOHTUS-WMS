...
from nohtus.db import exec_sql
import pandas as pd
...
def import_customer_master_excel(uploaded_file):
    ...
    rename = {'거래처코드': 'customer_code', '코드': 'customer_code', '거래처명': 'customer_name', '거래처': 'customer_name', '상호': 'customer_name', '사업장': 'company', '법인': 'company', '유형': 'customer_type', '거래처유형': 'customer_type', '담당자': 'manager', '담당': 'manager', '연락처': 'phone', '전화번호': 'phone', '핸드폰': 'phone', '주소': 'address', '납품처': 'address', '배송지': 'address', '메모': 'memo', '비고': 'memo'}
...
def render_inventory_metadata_editor():
    ...
    for r in inv.itertuples(index=False):
        ...
        labels.append(f"#{int(getattr(r, 'id'))} / {getattr(r, 'company')} / {getattr(r, 'location')} / {getattr(r, 'product_name')} / {wh} / LOT:{getattr(r, 'lot') or '-'} / EXP:{display_date_only(getattr(r, 'exp_date') or '-')} / {int(getattr(r, 'qty') or 0)}EA")
...
def apply_standard_name_change(old_name, new_name):
    ...
    with connect() as con:
        cur = con.cursor()
        for table, col in [('inventory', 'product_name'), ('transactions', 'product_name'), ('outbound_order_items', 'product_name'), ('erp_ambiguous_candidates', 'candidate_product')]:
            try:
...
def find_mapping_conflicts_from_inventory():
    ...
    for r in inv.itertuples():
        ...
        if other_names:
            rows.append({'사업장': company, '원본명 구분': label, 'ERP명/비자료명': source_name, '재고 DB 표준제품명': current_standard, '제품매칭표 기존 표준제품명': ' / '.join(mapped_names), '충돌 가능 내용': f"재고 DB는 '{current_standard}'로 쓰고 있지만 제품매칭표에는 '{source_name}'이(가) 다른 표준제품명에도 등록되어 있습니다.", '현재재고수량': int(r.qty or 0), '재고위치': str(r.locations or '')})
...
def update_inventory_metadata(inv_id, new_lot, new_exp, memo=''):
    ...
    with connect() as con:
        ...
        reason += merge_note
        insert_transaction_log(cur, created_at=now, tx_type='재고정보수정', product_name=src['product_name'], warehouse_name=src['warehouse_name'], lot=lot2, exp_date=exp2, from_company=src['company'], from_location=src['location'], to_company=src['company'], to_location=src['location'], qty=0, memo=reason)
        con.commit()
        return True
