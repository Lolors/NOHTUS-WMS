...
def page_erp_stock_compare():
    ...
    if run_compare:
        ...
        if wms_raw.empty:
            ...
            wms_sum = wms_raw.groupby(['사업장', '제품명'], as_index=False)['수량'].sum().rename(columns={'수량': 'WMS수량'})
    ...
    if 'erp_compare_summary' in st.session_state:
        ...
        if not comp.empty:
            ...
            st.download_button('비교 결과 엑셀 다운로드', data=dataframe_to_excel_bytes(comp, 'ERP_WMS_비교'), file_name=f"NOHTUS_ERP_WMS_비교_{date.today().strftime('%Y%m%d')}.xlsx", mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', use_container_width=True)
        if not amb.empty:
            st.warning('확인 필요 항목')
